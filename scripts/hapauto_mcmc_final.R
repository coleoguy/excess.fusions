...
dat <- read.csv("../data/chromes/dat.csv",
                as.is=T)[,c(1,3)]
tree <- force.ultrametric(read.nexus("../data/trees/cut.tree.nex"),method="extend")
...
save(model.mcmc,model.mcmc.postburn,file="../outputs/mcmc/hapauto_final.RData")
write.csv(parMat,
          paste0("../data/transition_matrix/Q_matrix_hapauto_final.csv"),
          row.names=F,quote=F)