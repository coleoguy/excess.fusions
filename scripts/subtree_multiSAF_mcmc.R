...
for(i in 1:5){
  ...
  # load in subtree
  split.tree <- force.ultrametric(read.nexus(paste0("../data/trees/subtrees/tree_",clades[i],".nex")),method="extend")
  ...
  for(j in 1:4){
    model.mcmc[[j]]<- diversitree::mcmc(lik=model.con,
                                        x.init=runif(2,0,1),
                                        prior=make.prior.exponential(r=0.5),
                                        #upper=c(100,100,100,100),
                                        nsteps = 500,
  ...
  #Save matrix
  save(model.mcmc,model.mcmc.postburn,file=paste0("../outputs/mcmc/subtrees/multiSAF/",clades[i],".RData"))
  write.csv(parMat,
